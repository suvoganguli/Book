...
if strncmpi(PitchSurf,'elev',4), 
  ...
  appendmws(struct('TrimWithStab',0),gtm_model);
  PitchSurfLim=EleLim;  
elseif strncmpi(PitchSurf,'stab',4)
    
  % Change 1C: suvo  
  %appendmws(struct('TrimWithStab',1),'gtm_design');
  appendmws(struct('TrimWithStab',1),gtm_model);
  PitchSurfLim=StbLim;
else
...
dirtyflag =get_param(gtm_model,'Dirty');
inlineflag=get_param(gtm_model,'InlineParams');
set_param(gtm_model,'InlineParams','off');
...
appendmws(struct('TrimModeOn',1),gtm_model);

% The four lines below were already commented in V0912: suvo
...
eval(['[tmp1,Xo,StateNames]=' gtm_model '([],[],[],' '''compile''' ');' ]);

% Find Configurable Subsystems

% Change 5: suvo
% AircraftType = get_param('gtm_design/Aircraft Model','BlockChoice');
AircraftType = get_param([gtm_model '/Aircraft Model'],'BlockChoice');
...
switch AircraftType
    case 'GTM_T2'
        ...
        Index.EOM  = find(strcmp(StateNames,[gtm_model '/Aircraft Model/GTM_T2/EOM/Integrator'])==1);
end
...
[XU_opt,opt] = simcnstr('trim',@(x) simio(x,Xo,Index,target,AircraftType),XU_0,para,XU_lower,XU_upper,[]);
...
appendmws(struct('TrimModeOn',0),gtm_model);
...
% gtm_design([],[],[],'term');
eval([ 'yout=' gtm_model '(0,Xtrim,[],' '''outputs''' ');' ]);
eval([ 'dx=' gtm_model '(0,Xtrim,[],' '''derivs''' ');' ]);
eval([ gtm_model '([],[],[],' '''term''' ');' ]);

% The two lines below were already commented in V0912: suvo
...
set_param(gtm_model,'InlineParams',inlineflag);
set_param(gtm_model,'Dirty',dirtyflag);


%============================== SubFunctions ==============================
function [f,g]=simio(xu,Xo,Index,target,AircraftType)
...
eval([ 'yout = ' gtm_model '(trim_at_time,Xtweak,[],' '''outputs''' ');' ]);
eval([ 'dx   = ' gtm_model '(trim_at_time,Xtweak,[],' '''derivs''' ');' ]);
