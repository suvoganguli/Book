...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_xdot_obac_process_check_sum_call( int nlhs, mxArray *
  ...
  extern unsigned int sf_engine_T2_lib_process_check_sum_call( int nlhs, mxArray
    * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_xdot_obac_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
...
{
  extern unsigned int sf_xdot_obac_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_xdot_obac_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_xdot_obac_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "xdot_obac") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_xdot_obac_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_xdot_obac_method_dispatcher(SimStruct *S, unsigned int
...
{
  if (!strcmp(machineName,"xdot_obac")) {
    return(sf_xdot_obac_method_dispatcher(simstructPtr,chartFileNumber,method,
            data));
  }
...
extern void xdot_obac_terminator(void);
...
extern void xdot_obac_initializer(void);
...
extern void xdot_obac_register_exported_symbols(SimStruct* S);
...
extern void xdot_obac_debug_initialize(void);
...
{
  xdot_obac_register_exported_symbols(S);
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      xdot_obac_debug_initialize();
    }

    xdot_obac_initializer();
    if (!simModeIsRTWGen) {
