...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_main_sim_adapt_process_check_sum_call( int nlhs,
  ...
  extern unsigned int sf_GTM_aerolib_process_check_sum_call( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_main_sim_adapt_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
...
{
  extern unsigned int sf_main_sim_adapt_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_main_sim_adapt_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_main_sim_adapt_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "main_sim_adapt") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_main_sim_adapt_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_main_sim_adapt_method_dispatcher(SimStruct *S, unsigned
...
{
  if (!strcmp(machineName,"main_sim_adapt")) {
    return(sf_main_sim_adapt_method_dispatcher(simstructPtr,chartFileNumber,
            method,data));
  }
...
extern void main_sim_adapt_terminator(void);
...
extern void main_sim_adapt_debug_initialize(void);
...
{
  main_sim_adapt_register_exported_symbols(S);
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      main_sim_adapt_debug_initialize();
    }

    main_sim_adapt_initializer();
    if (!simModeIsRTWGen) {
