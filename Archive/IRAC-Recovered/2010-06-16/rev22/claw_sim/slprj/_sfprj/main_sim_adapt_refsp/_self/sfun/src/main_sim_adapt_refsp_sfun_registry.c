...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_main_sim_adapt_refsp_process_check_sum_call( int nlhs,
  ...
  extern unsigned int sf_GTM_aerolib_process_check_sum_call( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_main_sim_adapt_refsp_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
...
{
  extern unsigned int sf_main_sim_adapt_refsp_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_main_sim_adapt_refsp_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_main_sim_adapt_refsp_get_eml_resolved_functions_info
  ...
  if (strcmp(machineName, "main_sim_adapt_refsp") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_main_sim_adapt_refsp_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_main_sim_adapt_refsp_method_dispatcher(SimStruct *S,
...
{
  if (!strcmp(machineName,"main_sim_adapt_refsp")) {
    return(sf_main_sim_adapt_refsp_method_dispatcher(simstructPtr,
...
extern void main_sim_adapt_refsp_terminator(void);
...
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    main_sim_adapt_refsp_terminator();
    models_terminator();
...
extern void main_sim_adapt_refsp_initializer(void);
extern void models_initializer(void);
extern void aeroutilslib_initializer(void);
extern void GTM_aerolib_initializer(void);
extern void main_sim_adapt_refsp_register_exported_symbols(SimStruct* S);
...
extern void main_sim_adapt_refsp_debug_initialize(void);
...
{
  main_sim_adapt_refsp_register_exported_symbols(S);
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      main_sim_adapt_refsp_debug_initialize();
    }

    main_sim_adapt_refsp_initializer();
    if (!simModeIsRTWGen) {
