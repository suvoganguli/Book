...
if (exist('MWS','var') && ~isempty(MWS))
    if isstruct(MWS)
        % loadmws(MWS);
        loadmws(MWS,gtm_model); % to load into a model different from gtm_design.mdl
    else
...
eval(['[tmp1,tmp2,StateNames]=' gtm_model ';']);
...
EOM =find(strcmp(StateNames,[[gtm_model '/Aircraft Model/'] AircraftType '/EOM/Integrator'])==1);

% Set LinearizationFlag

% Change 3: suvo
% appendmws(struct('LinearizeModeOn',1),'gtm_design');
appendmws(struct('LinearizeModeOn',1),gtm_model);
...
[a,b,c,d]=dlinmod(gtm_model,Ts,Xo,Args);
...
set(sys,'StateNames',Snames,'OutputNames',Onames,'InputNames',Inames);
...
appendmws(struct('LinearizeModeOn',0),gtm_model);
