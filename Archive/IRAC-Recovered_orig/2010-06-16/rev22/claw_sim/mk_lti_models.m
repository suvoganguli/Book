% mk_lti_models.m
%
% Original linmod states     : p,q,r, u,v,w, N,E,h, e0,e1,e2,e3 
%                             (vars:  a,b,c,d,tmpd)
% States for LTI MACH design : p,q,r, V,alpha,beta, N,E,h, exx, phi,theta,psi   
%                             (vars:  f,g,h,d2,tt)
% States for documentation   : p,q,r, V,alpha,beta, N,E,h, exx, phi,theta,psi
% 
% States for MRAC design     : p,q,r, u,v,w, N,E,h, exx, phi,theta,psi
%                             (vars:  a_mrac,b_mrac,c_mrac_d_mrac,tt1)
%
% Outputs - 
%   For documentation:
%       ix_LG = [ 2 4 5 9 12 ] ;     

disp('Note: Make sure that fc is used in rate loop');

do_GTM_expts

%% Adapted from dfemklin
...
time_linmod = input('Enter time instance for linearization');
...
% A new state variable for the non-minimal quaternion norm state is derived
% as follows
    % a * tmpvec = 0 * tmpvec
    % x_old = eigvec * x_new
...
% x = [p,q,r, u,v,w, N,E,h, exx, phi,theta,psi]

sys_mrac = ss2ss(sys,tt1) ;
[a_mrac,b_mrac,c_mrac,d_mrac] = ssdata(sys_mrac) ;
...
%save fc01_V0912_dc3.mat a b c d tt tt1
